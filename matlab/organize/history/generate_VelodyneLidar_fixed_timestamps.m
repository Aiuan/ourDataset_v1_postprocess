...
output_folder = './VelodyneLidar_fixed_timestamps';
...
function process_single(output_folder, dataset_process_path, dataset_group, OVERWRITE_ON)
    ...
    if ~exist(file_save_path, 'file') || OVERWRITE_ON
        VelodyneLidar_timestamps = get_VelodyneLidar_timestamps(fullfile(dataset_process_path, dataset_group));
...
function VelodyneLidar_timestamps = get_VelodyneLidar_timestamps(dataset_group_path)
    VelodyneLidar_timestamps = [];
    
    if exist(fullfile(dataset_group_path, 'VelodyneLidar_fixed'))
        temp = dir(fullfile(dataset_group_path, 'VelodyneLidar_fixed', '*.pcd'));
%     elseif exist(fullfile(dataset_group_path, 'VelodyneLidar'))
