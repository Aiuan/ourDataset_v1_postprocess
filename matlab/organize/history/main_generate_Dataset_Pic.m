...
LeopardCamera1_timestamps_offset = -0.1; % s
...
for i = 1:length(match_results)
    ...
    for j = 1:length(match_results(i).group)
        ...
        if ~OVERWRITE_ON
            if exist(fullfile(output_Pic_Projections_folder, sprintf('groupId%04d_frameId%04d.jpg', groupId, frameId)), 'file') && exist(fullfile(output_Pic_Sensors_folder, sprintf('groupId%04d_frameId%04d.jpg', groupId, frameId)), 'file') && exist(fullfile(output_Dataset_folder, sprintf('group%04d_frame%04d', groupId, frameId)), 'dir')
        ...
        if GenPic_ON
            showScence(PROJECT_ON, output_Pic_Sensors_folder, output_Pic_Projections_folder, groupId, frameId,...
                LeopardCamera1_png_path, VelodyneLidar_pcd_path, OCULiiRadar_pcd_path, TIRadar_pcd_path,...
                LeopardCamera1_IntrinsicMatrix, VelodyneLidar_to_LeopardCamera1_TransformMatrix, OCULiiRadar_to_LeopardCamera1_TransformMatrix, TIRadar_to_LeopardCamera1_TransformMatrix);
        end
...
function showScence(PROJECT_ON, output_Pic_Sensors_folder, output_Pic_Projections_folder, groupId, frameId,...
    LeopardCamera1_png_path, VelodyneLidar_pcd_path, OCULiiRadar_pcd_path, TIRadar_pcd_path,...
    LeopardCamera1_IntrinsicMatrix, VelodyneLidar_to_LeopardCamera1_TransformMatrix, OCULiiRadar_to_LeopardCamera1_TransformMatrix, TIRadar_to_LeopardCamera1_TransformMatrix)
...
function createDatasetFolders(OVERWRITE_ON, output_Dataset_folder, groupId, frameId,...
    ...
    VelodyneLidar_timestamp = round(str2double(temp), 3);
    ...
    if ~exist(VelodyneLidar_json_dstPath, 'file') || OVERWRITE_ON == 1
        ...
        VelodyneLidar_json.ros_timestamp = temp; % ros timestamp
