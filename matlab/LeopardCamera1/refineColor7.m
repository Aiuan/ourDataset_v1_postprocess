function image_refineColor = refineColor7(img_rgb)
    fprintf("refineColor7");     
    R = img_rgb(:,:,1);
    B = img_rgb(:,:,3);
    img_rgb(:,:,1) = B;
    img_rgb(:,:,3) = R;
    ...
    if RGBmean <= 0.2
        ...
        img_hsv(:,:,3) = (6-RGBmean*25) * img_hsv(:,:,3);
...
%          final_img = final_img;
        ...
        if G1-RBmean>=0.1
            Reference = 6-G1*2+(G1-RBmean)*15;
            brightness = 10+G1*2-(G1-RBmean)*15;
         end 
%          Reference = 6;
%          brightness = 10;   
...
%      RGB3 = [R1 G1 B1];
     if RGBmean > 0.9 || R1>1 || G1>1 ||B1>1
        ...
        img_hsv(:,:,3) = (max(1.93-RGBmean,0.55)) * img_hsv(:,:,3);
